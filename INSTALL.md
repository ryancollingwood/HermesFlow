...
| `--with-ollama` | off | Add a local Ollama container by layering [`docker-compose.ollama.yml`](docker-compose.ollama.yml) on via `COMPOSE_FILE`. |
| `--external-ollama <url>` | — | Point `HINDSIGHT_LLM_BASE_URL`/`BASEROW_OLLAMA_HOST` at an Ollama already running elsewhere (e.g. `http://host.docker.internal:11434` for one on the Docker host) instead of starting the bundled container. Mutually exclusive with `--with-ollama`. |
...
before the stack starts. `--with-ollama` layers
...
`HINDSIGHT_LLM_BASE_URL` / `BASEROW_OLLAMA_HOST` at an Ollama you already run
elsewhere; it's mutually exclusive with `--with-ollama`. `--with-baserow` layers
...
Only runs when `--with-ollama` is passed (or implied by `--gpu`). Adds
...
The same thing `make ollama` does; revert (and drop the override) with
`make ollama-revert`. Pass `--external-ollama <url>` instead to skip the
bundled container and point `HINDSIGHT_LLM_BASE_URL` / `BASEROW_OLLAMA_HOST`
at an Ollama already running elsewhere. See
