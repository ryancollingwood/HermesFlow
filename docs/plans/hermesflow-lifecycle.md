...
| HF-006 Audit/restrict Hermes direct execution | Inventory Hermes's execution-capable tools; guard/scoping for HermesFlow sessions; clear Windmill-unavailable message | Test by attempting shell/python/browser/fs through HermesFlow mode — **confirmed necessary, not theoretical** by HF-005's live testing above: a prompt-level skill states Rule 1 but cannot enforce it: the model chose the built-in `web_extract` tool over refusing. HF-006's tool-level restriction is what makes Rule 1 non-optional rather than advisory |
