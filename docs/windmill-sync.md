...
| `f/hermes_flow/{folder.meta.yaml,catalogue/models.*,catalogue/search.*,policies/evaluator.*,candidate_ops/models.*,candidate_ops/create.*,candidate_ops/diff.*,candidate_ops/promote.*,candidate_ops/prepare_promotion.*,candidate_ops/promotion.flow/**,candidate_ops/lifecycle.*,testing/runner.*,testing/example_test.*,testing/regression.*,testing/scheduled_health.*,testing/source_drift_fixture.*,repair/folder.meta.yaml,repair/models.*,repair/inspection.*,repair/generate_candidate.*,repair/promote_fixture.*}` | HermesFlow's own control-plane: capability catalogue/search/policy (HF-008–010), candidate lifecycle (HF-011–014), testing/regression/health scheduling (HF-015–016, HF-020), and bounded repair inspection/generation/fixture promotion (HF-029–031) | ✅ yes | ✅ yes (named explicitly, not wildcarded — see below) |
...
- **`includes`** — see [`wmill.yaml`](wmill.yaml) for the live list. `f/hermes/**`,
  ...
  - `f/hermes_flow/` (currently `f/hermes_flow/folder.meta.yaml`,
    ...
    `f/hermes_flow/testing/{runner,example_test,regression,scheduled_health,source_drift_fixture}.*`, plus
    `f/hermes_flow/repair/{folder.meta.yaml,models.*,inspection.*,generate_candidate.*,promote_fixture.*}`, and
...
| `f/hermes_flow/repair/` tracked metadata/models/inspection/generation/fixture-promotion assets | created / overwritten / removed with the same guarded mirror semantics; unenumerated repair assets remain untouched |
...
| `f/hermes_flow/repair/` tracked metadata/models/inspection/generation/fixture-promotion assets | written / overwritten / deleted locally to mirror the server; unenumerated repair assets are not pulled |
...
candidate-generation, and fixture-promotion operations participate in drift
checks; any other repair asset remains out of scope. Sanitised fixture bytes
remain in `/shared/artifacts`, outside Windmill sync scope.
