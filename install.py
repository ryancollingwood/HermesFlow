...
Other optional channels / toggles:
    ...
    --with-joshu                          add Joshu (AI cloud desktop) — needs the image
                                          built first from a joshu-oss clone (make joshu-build)
...
def compose_add(override: str) -> None:
    """Append an optional override file to COMPOSE_FILE, additively and
    idempotently (so --gpu and --with-baserow can both be active). Seeds the
    base docker-compose.yml when COMPOSE_FILE is unset."""
    cur = env_value("COMPOSE_FILE")
    parts = [p for p in cur.split(":") if p]
    if override in parts:
        return
    if not parts:
        parts = ["docker-compose.yml"]
    parts.append(override)
    env_set("COMPOSE_FILE", ":".join(parts))
...
def main() -> None:
    ...
    ap.add_argument("--with-joshu", action="store_true",
                    help="add Joshu (AI cloud desktop) via its compose override — the image must "
                         "be built first from a local joshu-oss clone (make joshu-build)")
    ...
    if args.dry_run:
        ...
        say("  Secrets:         generate any blank/weak of API_SERVER_KEY, WM_DB_PASSWORD, "
            "HINDSIGHT_DB_PASSWORD, GRAFANA_ADMIN_PASSWORD, BASEROW_*, JOSHU_*")
        ...
        say(f"  Joshu:           {yn(args.with_joshu)}"
            + (" (docker-compose.joshu.yml — image must be prebuilt: make joshu-build)"
               if args.with_joshu else ""))
    ...
    # Baserow (structured-data UI + REST API) — layer its optional compose
    # override. Secrets are generated by the secrets step below; AI fields default
    # to the local ollama container.
    if args.with_baserow:
    ...
    # Joshu (AI cloud desktop) — layer its optional compose override. There is
    # no public image: it is built locally from a joshu-oss clone
    # (make joshu-build). When the image is absent we deliberately do NOT layer
    # the override — the stack-wide `up` below would fail on the missing image —
    # and print the build path instead. Secrets (JOSHU_HERMES_API_KEY /
    # JOSHU_HERMES_DASHBOARD_PASSWORD) are generated by the secrets step below
    # regardless, so a later `make joshu` needs no extra steps.
    if args.with_joshu:
        joshu_ref = env_value("JOSHU_IMAGE_REF") or "joshu-oss:local"
        if run_ok(["docker", "image", "inspect", joshu_ref]):
            compose_add("docker-compose.joshu.yml")
            say(f"{OK} enabled Joshu — desktop at http://joshu.localhost "
                "(first boot takes 5-10 min; see docs/joshu.md)")
        else:
            say(f"{WARN} Joshu image '{joshu_ref}' not found — skipping the override for now")
            say("  (the stack would fail to start on the missing image). Build it from a local")
            say("  joshu-oss clone (JOSHU_SRC_DIR in .env, default ~/Projects/joshu-oss), then:")
            say("    git clone https://github.com/db-aeon/joshu-oss ~/Projects/joshu-oss")
            say("    make joshu-build     # 20-30+ min first time")
            say("    make joshu")
            say("  Details: docs/joshu.md")
