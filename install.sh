...
if [ "$DRY_RUN" -eq 1 ]; then
  ...
  echo "  Secrets:         generate any blank/weak of API_SERVER_KEY, WM_DB_PASSWORD, HINDSIGHT_DB_PASSWORD, GRAFANA_ADMIN_PASSWORD, COLLECTION_DB_ADMIN_PASSWORD, BASEROW_*, DIRECTUS_*, WINDMILL_COLLECTION_DB_PASSWORD, JOSHU_*"
  ...
  echo "  Joshu:           $(yn $WITH_JOSHU)$([ "$WITH_JOSHU" -eq 1 ] && echo " (docker-compose.joshu.yml — image must be prebuilt: make joshu-build)")"
  if [ -n "$EXTERNAL_OLLAMA" ]; then
...
# Joshu (AI cloud desktop) — layer its optional compose override. There is no
# public image: it is built locally from a joshu-oss clone (make joshu-build).
# When the image is absent we deliberately do NOT layer the override — the
# stack-wide `up` below would fail on the missing image — and print the build
# path instead. Secrets (JOSHU_HERMES_API_KEY / JOSHU_HERMES_DASHBOARD_PASSWORD)
# are generated by `make secrets` below regardless, so a later `make joshu`
# needs no extra steps.
if [ "$WITH_JOSHU" -eq 1 ]; then
  JOSHU_REF="$(grep -E '^JOSHU_IMAGE_REF=' .env 2>/dev/null | head -1 | cut -d= -f2- | xargs || true)"
  JOSHU_REF="${JOSHU_REF:-joshu-oss:local}"
  if docker image inspect "$JOSHU_REF" >/dev/null 2>&1; then
    compose_add docker-compose.joshu.yml
    echo "✓ enabled Joshu — desktop at http://joshu.localhost (first boot takes 5-10 min; see docs/joshu.md)"
  else
    echo "⚠ Joshu image '$JOSHU_REF' not found — skipping the override for now (the stack"
    echo "  would fail to start on the missing image). Build it from a local joshu-oss"
    echo "  clone (JOSHU_SRC_DIR in .env, default ~/Projects/joshu-oss), then enable it:"
    echo "    git clone https://github.com/db-aeon/joshu-oss ~/Projects/joshu-oss"
    echo "    make joshu-build     # 20-30+ min first time"
    echo "    make joshu"
    echo "  Details: docs/joshu.md"
  fi
fi
