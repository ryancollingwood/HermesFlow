...
| inherited resource-types (the ~250 from `admins`) | Windmill built-ins | ❌ no | ❌ no |
| secret variables (anywhere) | credentials | ❌ no (placeholder only) | ❌ no (`skipSecrets`) |

Two settings enforce this:

- **`includes: ["f/hermes/**", "hermes_endpoint.resource-type.yaml"]`** — only the
  Hermes code folder and the one resource-type. The glob is separator-aware, so
  `f/hermes/**` does **not** match `f/hermes_state` or any other folder.
...
| Item | What happens |
|---|---|
| `f/hermes` item in repo **and** server | server version **overwritten** |
| `f/hermes` item in repo, **not** on server | **created** on server |
| `f/hermes` item on server, **not** in repo | **removed** — scripts **archived** (recoverable), resources/variables/folders **hard-deleted** — *the dry-run guard aborts here unless `FORCE=1`* |
| `f/hermes/folder.meta.yaml` | tracked → pushed (folder perms preserved); untracked → deleted |
...
| Item | What happens |
|---|---|
| `f/hermes` on server **and** in repo | repo file **overwritten** |
| `f/hermes` on server, **not** in repo | **written** into `windmill/f/hermes/` |
| `f/hermes` in repo, **not** on server | **deleted** from working tree (`git checkout` restores) |
...
| secret variables under `f/hermes` | **placeholder** only (real value never written) |
...
`f/hermes_state` and other folders **never show as drift** — their divergence is
